# on change uniquement la fonction is_not_possible, pour inclure la carte joker dans les caractères possibles
...
def is_not_possible(tirage, word):
    """
    Check if word is possible function.
    Returns True if the given word can be formed using the given letters,
    otherwise returns False.
    """
    copied_tirage = list(tirage)
    joker_used = False
    for letter in word:
        if letter in copied_tirage:
            copied_tirage.remove(letter)
        else:
            if '?' in copied_tirage and not joker_used:
                copied_tirage.remove('?')
                joker_used = True
            else:
                return False
    return True
